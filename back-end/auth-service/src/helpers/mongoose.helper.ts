...
export const getOrder = (sorts: Sorting[]): Record<string, any> => {
    ...
    for (const sort of sorts) {
        orderByObj[sort.property] = sort.direction === 'asc' ? 1 : -1;
    }
    return orderByObj;
}
...
export const getWhere = (filters: Filtering[]): Record<string, any> => {
    ...
    for (const filter of filters) {
        ...
        if (filter.rule === FilterRule.IS_NOT_NULL) whereObj[filter.property] = { $ne: null };
        if (filter.rule === FilterRule.EQUALS) whereObj[filter.property] = filter.value;
        if (filter.rule === FilterRule.NOT_EQUALS) whereObj[filter.property] = { $ne: filter.value };
        ...
        if (filter.rule === FilterRule.LIKE) whereObj[filter.property] = { $regex: filter.value, $options: 'i' };
        if (filter.rule === FilterRule.NOT_LIKE) whereObj[filter.property] = { $not: { $regex: filter.value, $options: 'i' } };
        if (filter.rule === FilterRule.IN) whereObj[filter.property] = { $in: filter.value.split(',') };
        if (filter.rule === FilterRule.NOT_IN) whereObj[filter.property] = { $nin: filter.value.split(',') };
    };
    return whereObj;
};